...
function layerFactory(L) {
  let CanvasIconLayer = (L.Layer ? L.Layer : L.Class).extend({
    ...
    _redraw: function (clear) {
      ...
      self._latlngMarkers.search(mapBoxCoords).forEach(function (e) {
        // Readjust Point Map
        let pointPos = self._map.latLngToContainerPoint(e.data.getLatLng())
        let iconSize = e.data.options.icon.options.iconSize
        let adjX = iconSize[0] / 2
        let adjY = iconSize[1] / 2
        var newCoords = {
          minX: (pointPos.x - adjX),
          minY: (pointPos.y - adjY),
          maxX: (pointPos.x + adjX),
